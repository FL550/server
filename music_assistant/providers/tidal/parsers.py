...
from .constants import BROWSE_URL, RESOURCES_URL, STARRED_TRACKS_PLAYLIST_ID
...
def build_starred_tracks_playlist(provider: TidalProvider) -> Playlist:
    """Build a virtual Playlist representing the user's starred (favorited) tracks."""
    owner_name = str(provider.auth.user_id)
    if provider.auth.user.profile_name:
        owner_name = provider.auth.user.profile_name
    elif provider.auth.user.user_name:
    ...
    return Playlist(
        item_id=STARRED_TRACKS_PLAYLIST_ID,
        provider=provider.instance_id,
        name="Starred Tracks",
        owner=owner_name,
        provider_mappings={
